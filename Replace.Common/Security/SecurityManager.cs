...
namespace Replace.Common.Security
{
    public class SecurityManager
    {
        ...
        private void Handshake(ushort packetID, PacketReader packet_data, bool packet_encrypted)
        {
            if (packet_encrypted)
            {
                throw (new SecurityException("SecurityManager::Handshake->Received an illogical (encrypted) handshake packet."));
            }
            if (m_client_security)
            {
                ...
                if (m_security_flags.handshake == 0)
                {
                    ...
                    if (packetID == 0x9000)
                    {
                        ...
                        {
                            throw (new SecurityException("SecurityManager::Handshake->Received an illogical handshake packet (duplicate 0x9000)."));
                        }
                        m_accepted_handshake = true; // Otherwise, all good here
                        return;
                    }
                    // Client should not send any 0x5000s!
                    else if (packetID == 0x5000)
                    {
                        throw (new SecurityException("SecurityManager::Handshake->Received an illogical handshake packet (0x5000 with no handshake)."));
                    }
                    // Programmer made a mistake in calling this function
                    else
                    {
                        throw (new SecurityException("SecurityManager::Handshake->Received an illogical handshake packet (programmer error)."));
                    }
                }
                else
                {
                    ...
                    if (packetID == 0x9000)
                    {
                        ...
                        {
                            throw (new SecurityException("SecurityManager::Handshake->Received an illogical handshake packet (out of order 0x9000)."));
                        }
                    ...
                    else if (packetID == 0x5000)
                    {
                        ...
                        {
                            throw (new SecurityException("SecurityManager::Handshake->Received an illogical handshake packet (duplicate 0x5000)."));
                        }
                ...
                {
                    throw (new SecurityException("SecurityManager::Handshake->Client signature error."));
                }
            ...
            else
            {
                ...
                if (flags.handshake_response == 1)
                {
                    ...
                    {
                        throw (new SecurityException("SecurityManager::Handshake->Server signature error."));
                    }
                    ...
                    foreach (TransferBuffer buffer in incoming_buffers_tmp)
                    {
                        ...
                        if (m_client_security)
                        {
                            if (m_security_flags.security_bytes == 1)
                            {
                                ...
                                if (packet_security_count != expected_count)
                                {
                                    ...
                                    //Logger.Warn($"[SecurityAPI::Recv] Count byte mismatch on {packet_opcode.ToString("X4")}. (Expected: {expected_count}, Packet: {packet_security_count})");
                                }
                                ...
                                if (packet_security_crc != expected_crc)
                                {
                                    ...
                                    //Logger.Warn($"[SecurityAPI::Recv] CRC byte mismatch on {packet_opcode.ToString("X4")}. (Expected: {expected_crc}, Packet: {packet_security_crc})");
                                }
                        ...
                        else
                        {
                            if (m_client_security)
                            {
                                ...
                                if (!m_accepted_handshake)
                                {
                                    throw (new SecurityException("SecurityManager::Recv->The client has not accepted the handshake."));
                                }
                            }

                            //Send out our Identity
                            if (m_identity_flag == 1 && packet_opcode == 0x2001)
                            {
                                if (m_identity_send == true)
                                {
                                    throw (new SecurityException("SecurityManager::Recv->Identity already send."));
                                }

                                Packet packet = new Packet(packet_opcode, true);
                                packet.WriteAscii(m_identity_name);
                                packet.WriteByte(m_identity_flag);
                                packet.Lock();
                                m_outgoing_packets.Add(packet);

                                m_identity_send = true;
                            }
                            ...
                            if (packet_opcode == 0x600D)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        throw (new SecurityException("SecurityManager::Recv->A malformed 0x600D packet was received."));
                                    }
